...
\mysubsubsection{Errors of floating points}
...
For the error introduced by one base operation $a_i\times cos(b_i + c_i \times t)$, we must decompose it:
\begin{itemize}
...
\item the error bound of $cos(b_i+c_i\times t)$ is thus $ulp(b_i) + 5\,ulp(c_i) + (0.5 or 1)\,ulp(b_i+c_i\times t)+ ulp(cos(b_i+c_i\times t))$
\end{itemize}

The total error bound for a base operation is thus $$ulp(a_i) + ulp(b_i) + 5\,ulp(c_i) + (0.5 or 1)\,ulp(b_i+c_i\times t)+ ulp(cos(b_i+c_i\times t)) + \frac{1}{2}ulp(a_i\times cos(b_i+c_i\times t))$$

In the worst case, all these ulp are with the same exponent $e$. In this case we have the precision of the basic operation being 9.5 or 10 ulp($e$)TODO

, hence a maximum of $\rfloor log_2(10)\rfloor = 3$ corrupted digits.

In the big sums, we usually have a very proeminent first $a_1$ and starting from the second one, we have $a_i<\frac{a_1}{10}$. This means that the three last digits of all $a_i$ after the first will be lost. So we can assume that only three digits are corrupted in the final result if we make the sum in the ascending order and with the Kahan formula.

Now if we take into account the polynomial evaluation, we have the same kind phenomenon as described just before: only one term will prevail, other terms' last digits won't be taken into account.

At the end, we have a very good accuracy with only 3 corrupted digits.
