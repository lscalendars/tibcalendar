...
void
print_year_info (FILE * f, tib_year * year, astro_system * asys)
{
...
void
print_month_info (FILE * f, tib_month * month, astro_system * asys)
{
...
void
print_day_info (FILE * f, tib_day * day, astro_system * asys)
{
...
// if adj_mth is the last month of previous year, we take the following one
  if (adj_mth == 12L)		// TODO: test for 1935, 2000 (Phugpa)
	...
	printf
	  ("strange error(12): please report it to the tibastro developpers.");
    }
  ...
  td->month = month;
  get_month_data (asys->epoch, zd[0], month->rilcha, month->nyidru,
		  month->gzadru);
  // TODO: test the day loop for Phugpa 1977: first day is ommited
  // now we look for the first day
  td->gd =
    get_tt_data (asys->epoch, zd[0], month->gzadru, month->nyidru,
		 month->rilcha, 1, td->nyidag, td->gzadag, td->nyibar);
  // we have our day
  ...
  if (prv_gd == td->gd)		// means that the lunar date is ommited, we go to the next one
    {
      ...
      td->gd =
	get_tt_data (asys->epoch, zd[0], month->gzadru, month->nyidru,
		     month->rilcha, 2, td->nyidag, td->gzadag, td->nyibar);
      td->ommited = PREVIOUS_OMMITED;
      nxt_gd =
	get_tt_data (asys->epoch, zd[0], month->gzadru, month->nyidru,
		     month->rilcha, 3, nyidag, gzadag, nyibar);
    }
  else if (td->gd - prv_gd == 2)	// means that the lunar date is duplicated, we go to the first one
    {
      ...
      nxt_gd =
	get_tt_data (asys->epoch, zd[0], month->gzadru, month->nyidru,
		     month->rilcha, 2, nyidag, gzadag, nyibar);
    }
  else
    {
      nxt_gd =
...
void
print_tib_cal (FILE * f, int year, astro_system * asys)
{
